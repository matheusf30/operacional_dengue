...
def tratar_focos(focos):
	...
	focos_semanal_pivot = focos_semanal_pivot.reindex(pd.date_range(start = primeira_semana,
																	end = semana_atual,
																	freq = "W-SUN"))
	focos_semanal_pivot = focos_semanal_pivot.fillna(0)
	focos_semanal_pivot.reset_index(inplace = True)
	focos_semanal_pivot.rename(columns = {"index": "Semana"}, inplace = True)
	for c in focos_semanal_pivot.columns:
		if c != "Semana":
			focos_semanal_pivot[c] = focos_semanal_pivot[c].astype(int)
...
print(f"\n{green}FOCOS ATUALIZADOS (17 Regionais):\n{reset}{focos_semanal_pivot}\n")
print(f"\n{green}FOCOS ATUALIZADOS (17 Regionais - Colunas):\n{reset}{focos_semanal_pivot.columns}\n")
...
#sys.exit()
focos_pivot = pd.read_csv(f"{caminho_dados}focos_pivot.csv")
focos_pivot["Semana"] = pd.to_datetime(focos_pivot["Semana"])
#focos_semanal_pivot = focos_semanal_pivot.merge(focos_semana_pivot, on = "Semana", how = "outer")
focos_semanal_pivot = focos_semanal_pivot.drop(columns = "semana")
focos_pivot = pd.concat([focos_pivot, focos_semanal_pivot], axis = 0, ignore_index = True)
focos_pivot = focos_pivot.drop_duplicates(subset = ["Semana"])
focos_pivot["Semana"] = pd.to_datetime(focos_pivot["Semana"])
focos_pivot = focos_pivot.sort_values("Semana").reset_index(drop=True)
focos_pivot = focos_pivot.fillna(0)
colunas = focos_pivot.columns.drop("Semana")
focos_pivot[colunas] = focos_pivot[colunas].astype(int)
print(f"\n{green}FOCOS CONSOLIDADOS (17 Regionais):\n{reset}{focos_pivot}\n")
print(f"\n{green}FOCOS CONSOLIDADOS (17 Regionais - Colunas):\n{reset}{focos_pivot.columns}\n")
tempo_epidemiologico(focos_pivot)
focos_pivot.to_csv(f"{caminho_dados}focos_pivot.csv", index = False)
print(f"\n{green}SALVANDO FOCOS (semanal):\n{reset}{caminho_dados}{focos_pivot}\n")
#sys.exit()
