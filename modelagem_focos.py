...
dataset = dataset.merge(focos[["Semana", _CIDADE]], how = "right", on = "Semana").copy()
troca_nome = {f"{_CIDADE}_x" : "PREC", f"{_CIDADE}_y" : "FOCOS"}
dataset = dataset.rename(columns = troca_nome)
dataset["FOCOS"] = dataset["FOCOS"].astype(int)
...
def monta_dataset(_CIDADE):
	...
	dataset = dataset.merge(focos[["Semana", _CIDADE]], how = "right", on = "Semana").copy()
	dataset = dataset.rename(columns = {_CIDADE : "FOCOS"})
...
def treino_teste(dataset, _CIDADE):
    SEED = np.random.seed(0)
    x = dataset.drop(columns = "FOCOS").copy()
    y = dataset["FOCOS"]
...
def grafico_previsao(teste, previsao, string_modelo, _CIDADE):
    ...
    for velho, novo in troca.items():
        _CIDADE = _CIDADE.replace(velho, novo)
    #plt.savefig(f'{caminho_resultados}verificatualizacao_modelo_RF_focos_{_CIDADE}_{_LIMITE}-{_FIM}.pdf', format = "pdf", dpi = 1200)
    plt.show()
...
def histograma_erro(teste, previsao):
    ...
    ax = plt.gca()
    ax.set_facecolor("honeydew")
    sns.histplot(final["Focos"], bins = 50, kde = True, color = "blue")
...
def boxplot_erro(teste, previsao):
    ...
    ax.set_facecolor("honeydew")
    posicao = [1, 2, 3]
    plt.boxplot([final["Focos"], final["Previstos"], final["Erro"]], positions = posicao)
...
def salva_modelo(string_modelo, modeloNN = None):
    if string_modelo not in ["RF", "NN"]:
        ...
        if modeloNN is None:
            ...
            modeloNN.save(modeloNN, f"{caminho_modelos}NN_focos_r{_RETROAGIR}_{_CIDADE}.h5")
    else:
...
dataset2 = dataset2.merge(focos[["Semana", _CIDADE]], how = "right", on = "Semana").copy()
troca_nome = {f"{_CIDADE}_x" : "PREC", f"{_CIDADE}_y" : "FOCOS"}
dataset2 = dataset2.rename(columns = troca_nome)
dataset2["FOCOS"] = dataset2["FOCOS"].astype(int)
for r in range(_HORIZONTE + 1, _RETROAGIR):
	...
	#dataset[f"FOCOS_r{r}"] = dataset["FOCOS"].shift(-r)
#for r in range(2, 5):
	dataset2[f"FOCOS_r{r}"] = dataset2["FOCOS"].shift(-r)
...
sns.lineplot(x = x2.index, y = previsoes2,
             color = "red", alpha = 0.7, linewidth = 3, label = "Previsto")
sns.lineplot(x = x2.index, y = x2["FOCOS"],
             color = "blue", alpha = 0.9, linewidth = 1, label = "Observado")
plt.title(f"MODELO RANDOM FOREST: PREVISÃO DE FOCOS DE _Aedes_ sp.\n MUNICÍPIO DE {_CIDADE}, SANTA CATARINA.\n")
...
print(f"\n{green}dataset3:\n{reset}{dataset3}")
print(f"\n{green}dataset3.info:\n{reset}{dataset3.info()}")
dataset_focos = pd.DataFrame()
dataset_focos["Semana"] = dataset3["Semana"].iloc[-12:]
print(f"\n{green}dataset_focos:\n{reset}{dataset_focos}")
print(f"\n{green}dataset_focos.info:\n{reset}{dataset_focos.info()}")
dataset_focos = dataset_focos.merge(focos[["Semana", _CIDADE]], how = "left", on = "Semana").copy()
dataset_focos = dataset_focos.rename(columns = {_CIDADE : "FOCOS"})
print(f"\n{green}dataset_focos:\n{reset}{dataset_focos}")
print(f"\n{green}dataset_focos.info:\n{reset}{dataset_focos.info()}")
dataset_focos.loc[dataset_focos.index[-2], "FOCOS"] = previsoes2[-2]
dataset_focos.loc[dataset_focos.index[-1], "FOCOS"] = previsoes2[-1]
print(f"\n{green}dataset_focos:\n{reset}{dataset_focos}")
print(f"\n{green}dataset_focos.info:\n{reset}{dataset_focos.info()}")
#sys.exit()
dataset3 = dataset3.merge(dataset_focos[["Semana", "FOCOS"]], how = "right", on = "Semana").copy()
print(f"\n{green}dataset3:\n{reset}{dataset3}")
print(f"\n{green}dataset3.info:\n{reset}{dataset3.info()}")
for r in range(_HORIZONTE + 1, _RETROAGIR):
	...
	#dataset[f"FOCOS_r{r}"] = dataset["FOCOS"].shift(-r)
#for r in range(2, 5):
	dataset3[f"FOCOS_r{r}"] = dataset3["FOCOS"].shift(-r)
...
sns.lineplot(x = x3.index, y = previsoes3,
             color = "red", alpha = 0.7, linewidth = 3, label = "Previsto")
sns.lineplot(x = x3.index, y = x3["FOCOS"],
             color = "blue", alpha = 0.9, linewidth = 1, label = "Observado")
plt.title(f"MODELO RANDOM FOREST: PREVISÃO DE FOCOS DE _Aedes_ sp..\n MUNICÍPIO DE {_CIDADE}, SANTA CATARINA.\n")
plt.xlabel("Semanas Epidemiológicas")
plt.ylabel("Número de Focos de _Aedes_ sp.")
