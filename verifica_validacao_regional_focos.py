...
plt.plot(focos_atual["Semana"], focos_atual[_CIDADE],
				label = "Observado", color = "darkgreen")
plt.xlabel("Semanas Epidemiológicas (Série Temporal)")
plt.ylabel("Quantidade de $\it{Aedes}$ spp.") #Itálico by Everton
#plt.title("COMPARAÇÃO ENTRE FOCOS DE _Aedes_ spp. PREVISTOS E OBSERVADOS", fontsize = 12, y = 1.03, loc = "center", pad = 10)
plt.title("COMPARAÇÃO ENTRE QUANTIDADE DE $\it{Aedes}$ spp. PREVISTOS E OBSERVADOS", fontsize = 12, y = 1.03, loc = "center", pad = 10) #Aedes spp. em itálico by Everton
plt.suptitle(f"MUNICÍPIO DE {_CIDADE}", fontsize = 16, y = 0.96)
plt.legend(fontsize = 14)
#plt.ylim(0, casos_atual_reg.max().max())
ax = plt.gca()
