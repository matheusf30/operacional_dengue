...
#_ANO_MES = 202512
#_ANO_MES_DIA = 20251215 #f"{_ANO_ATUAL}{_MES_ATUAL}{_DIA_ATUAL}"
...
visu_preditivo = f"visualiza_preditivo_focos_vSE{SE}.csv"
previsao_total = f"previsao_total_focos_vSE{SE}.csv"
...
regionais = "censo_sc_regional.csv"
...
visu_preditivo = pd.read_csv(f"{caminho_resultados}{visu_preditivo}", low_memory = False)
previsao_total = pd.read_csv(f"{caminho_resultados}{previsao_total}", low_memory = False)
...
print(f"\n{green}VISUALIZAÇÃO PREDITIVA:\n{reset}{visu_preditivo}\n")
...
focos_atual["Semana"] = pd.to_datetime(focos_atual["Semana"], errors = "coerce") 
...
previsao_total = previsao_total.set_index("Semana")
previsao_total_reg = previsao_total.groupby(previsao_total.columns.map(mapeamento), axis = 1).sum()
...
#focos_atual_reg.reset_index(inplace = True)
previsao_total_reg.reset_index(inplace = True)
previstos.reset_index(inplace = True)
previsao12.reset_index(inplace = True)
focos_atual.reset_index(inplace = True)
focos_atual["Semana"] = pd.to_datetime(focos_atual["Semana"], errors = "coerce") 
focos_atual_reg.index = pd.to_datetime(focos_atual_reg.index, errors = "coerce") 
previsao_total["Semana"] = pd.to_datetime(previsao_total["Semana"], errors = "coerce")
previsao_total_reg["Semana"] = pd.to_datetime(previsao_total_reg["Semana"], errors = "coerce") 
...
previsao_pivot.reset_index(inplace = True)
...
print(f"\n{green}FOCOS_ATUAL_REG.dtypes:\n{reset}{focos_atual_reg}\n{focos_atual_reg.dtypes}\n")
print(f"\n{green}PREVISTOS.dtypes:\n{reset}{previstos}\n{previstos.dtypes}\n")
print(f"\n{green}previsao_total.dtypes:\n{reset}{previsao_total}\n{previsao_total.dtypes}\n")
print(f"\n{green}previsao_total_reg.dtypes:\n{reset}{previsao_total_reg}\n{previsao_total_reg.dtypes}\n")
...
#plt.plot(previsao_total["Semana"], previsao_total[_CIDADE],
#			label = "Previsto", color = "red", linewidth = 3)
...
plt.ylabel("Focos de $\it{Aedes}$ spp.") #Itálico by Everton
#plt.title("COMPARAÇÃO ENTRE FOCOS DE _Aedes_ spp. PREVISTOS E OBSERVADOS", fontsize = 12, y = 1.03, loc = "center", pad = 10)
plt.title("COMPARAÇÃO ENTRE FOCOS DE $\it{Aedes}$ spp. PREVISTOS E OBSERVADOS", fontsize = 12, y = 1.03, loc = "center", pad = 10) #Aedes spp. em itálico by Everton
...
if _VISUALIZAR == True:
	...
	plt.show()
if _SALVAR == True and _AUTOMATIZAR== True:
	for velho, novo in troca.items():
...
for idx, _REG in enumerate(regionais):
	...
	plt.plot(previsao_pivot_reg.index, previsao_pivot_reg[_REG],
				label = "Previsto", color = "red", linewidth = 3)
	#plt.plot(previsao_total_reg.index, previsao_total_reg[_REG],
	#			label = "Previsto", color = "red", linewidth = 3)
	plt.plot(focos_atual_reg.index, focos_atual_reg[_REG],
					label = "Observado", color = "darkgreen")
	#plt.plot(previsao_total_reg["Semana"], previsao_total_reg[_REG],
	#			label = "Previsto", color = "red", linewidth = 3)
	#plt.plot(focos_atual_reg["Semana"], focos_atual_reg[_REG],
	#				label = "Observado", color = "darkgreen")
	plt.xlabel("Semanas Epidemiológicas (Série Temporal)")
	plt.ylabel("Focos de $\it{Aedes}$ spp.") #Itálico by Everton
	#plt.title("COMPARAÇÃO ENTRE FOCOS DE _Aedes_ spp. PREVISTOS E OBSERVADOS", fontsize = 12, y = 1.03, loc = "center", pad = 10)
