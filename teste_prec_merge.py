...
def limite_minmax_anomalia(regiao_prec):
	...
	norm = cls.Normalize(vmin=-rounded_abs, vmax=rounded_abs)
...
def limite_colobar(regiao_prec):
	...
	if ((int_max - int_min)//2 != (int_max-int_min)/2):
		int_max += 1
	levels = range(int_min, int_max + 1, 4)
	levels2 = range(int_min, int_max + 1, 4)
	#levels = range(0, 60, 4)
	#levels2 = range(0, 60, 4)
...
def gerar_mapa(dataset, comportamento):
	...
	cbar = plt.colorbar(figure, fraction = 0.035, pad = 0.03, ticks = levels,
				label = "Precipitação (mm)", orientation = "vertical")
...
def main():
	"""
	Função principal. Executada ao ser chamado no terminal.
	*global libera acesso fora de funções, 'local'.
	*global retorna variáveis ao ponto 'global'
	"""
	global SE, _SALVAR, _VISUALIZAR, caminho_resultado, caminho_shapefile
	global sc_shape, regionais, levels, levels2, norm, cmap
...
### EXECUTANDO PELO PRÓPRIO NOME NA CHAMADA DO TERMINAL
