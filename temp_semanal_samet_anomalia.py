...
#################################################################################
# CAMINHOS E ARQUIVOS
caminho_samet = "/media/dados/operacao/samet/CDO.SAMET/"
...
arquivo_tmin = f"SAMeT_CPTEC_MONTHLY_TMIN_MEAN_ANOMALIA.nc"
arquivo_tmed = f"SAMeT_CPTEC_MONTHLY_TMED_MEAN_ANOMALIA.nc" 
arquivo_tmax = f"SAMeT_CPTEC_MONTHLY_TMAX_MEAN_ANOMALIA.nc" 
ds_tmin = xr.open_dataset(f"{caminho_samet}{arquivo_tmin}")
ds_tmed = xr.open_dataset(f"{caminho_samet}{arquivo_tmed}")
ds_tmax = xr.open_dataset(f"{caminho_samet}{arquivo_tmax}")
...
def gerar_mapa(dataset, str_var):
	...
	plt.colorbar(figure, fraction = 0.031, pad = 0.03, ticks = levels,
				label = "Anomalia de Temperatura Semanal (°C)",
	...
	match str_var:
		case "tmin":
			plt.title(f"Anomalia de Temperatura Mínima Média Semanal\nPeríodo observado: {_d7}",
						fontsize = 14, ha = "center")
		case "tmed":
			plt.title(f"Anomalia de Temperatura Média Semanal\nPeríodo observado: {_d7}",
						fontsize = 14, ha = "center")
		case "tmax":
			plt.title(f"Anomalia de Temperatura Máxima Média Semanal\nPeríodo observado: {_d7}",
						fontsize = 14, ha = "center")
		case _:
			print(f"\nVariável não encontrada!\n{str_var}\nVariável não encontrada!\n")		
	ax.add_geometries(shp, ccrs.PlateCarree(),
...
levels, norm = limite_minmax_anomalia(regiao_tmin)
gerar_mapa(regiao_tmin, "tmin")
levels, norm = limite_minmax_anomalia(regiao_tmed)
gerar_mapa(regiao_tmed, "tmed")
levels, norm = limite_minmax_anomalia(regiao_tmax)
