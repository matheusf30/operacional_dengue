...
# Common temperature anomaly ranges used in climatology
if abs_value <= 1:
    levels = np.arange(-1, 1.1, 0.2)  # Fine resolution for small anomalies
elif abs_value <= 3:
    levels = np.arange(-3, 3.1, 0.5)  # Standard for moderate anomalies
elif abs_value <= 5:
    levels = np.arange(-5, 5.1, 1.0)  # Coarser for larger anomalies
else:
    # For extreme anomalies, use logarithmic-like spacing
    levels = np.unique(np.concatenate([
        -np.logspace(0, np.log10(abs_value), 6)[::-1],
        [0],
        np.logspace(0, np.log10(abs_value), 6)
    ]))
