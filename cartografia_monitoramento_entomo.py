...
plt.title(f"Focos de $\\it{{Aedes}}$ spp. em Santa Catarina\nSemana Epidemiológica: {tempo['SE'].iloc[-2]}/{tempo['ano_epi'].iloc[-2]}.", fontsize = 14)
nome_arquivo = f"FOCOS_mapa_monitoramento_vSE{tempo['SE'].iloc[-1]}_{tempo['ano_epi'].iloc[-2]}_SE{tempo['SE'].iloc[-2]}.png"
...
base_carto.plot(ax = ax, column = "incidencia",  legend = True,
				edgecolor = "white", label = "Quantidade Ponderada de Focos de $\it{{Aedes}}$ spp./100 Mil Habitantes",
				legend_kwds = {"extend": "max", "fraction": 0.035, "pad": 0.03, "label": "Focos de $\it{{Aedes}}$ spp./100 Mil Habitantes"},
...
(Focos de $\it{{Aedes}}$ spp./100 mil habitantes)
...
ax.set_yticks([-29, -28, -27, -26])
ax.set_yticklabels(["29°S", "28°S", "27°S", "26°S"])
plt.title(f"Focos Ponderados* de $\it{{Aedes}}$ spp. em Santa Catarina\nSemana Epidemiológica: {tempo['SE'].iloc[-2]}/{tempo['ano_epi'].iloc[-2]}", fontsize = 14)
nome_arquivo = f"FOCOPONDERADO_mapa_monitoramento_vSE{tempo['SE'].iloc[-1]}_{tempo['ano_epi'].iloc[-2]}_SE{tempo['SE'].iloc[-2]}.png"
if _AUTOMATIZA == True and _SALVAR == True:
	os.makedirs(caminho_resultados, exist_ok = True)
	plt.savefig(f"{caminho_resultados}{nome_arquivo}", format = "png", dpi = 300,
