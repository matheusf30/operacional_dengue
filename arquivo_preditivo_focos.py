...
#_ANO_MES_DIA = "20251124" #Everton - Excluir Depois
...
if _LOCAL == "GH": # _ = Variável Privada
	...
	caminho_modelos = f"/home/meteoro/scripts/operacional_dengue/modelagem/focos/{_ANO_ATUAL}/{_ANO_MES_DIA}/"
...
def monta_dataset(cidade):
	...
	dataset = dataset.merge(focos[["Semana", cidade]], how = "right", on = "Semana").copy()
	dataset = dataset.rename(columns = {f"{cidade}" : "FOCOS"})
...
def monta_dataset2(cidade, previsoes):
	...
	dataset_focos["Semana"] = dataset2["Semana"].iloc[-12:]
	print(f"\n{green}dataset_focos:\n{reset}{dataset_focos}")
	...
	print(f"\n{green}dataset_focos:\n{reset}{dataset_focos}")
	print(f"\n{green}dataset_focos.info:\n{reset}{dataset_focos.info()}")
	dataset_focos.loc[dataset_focos.index[-3], "FOCOS"] = previsoes[-3]
	dataset_focos.loc[dataset_focos.index[-2], "FOCOS"] = previsoes[-2]
	dataset_focos.loc[dataset_focos.index[-1], "FOCOS"] = previsoes[-1]
	print(f"\n{green}dataset_focos:\n{reset}{dataset_focos}")
	print(f"\n{green}dataset_focos.info:\n{reset}{dataset_focos.info()}")
	#sys.exit()
	dataset2 = dataset2.merge(dataset_focos[["Semana", "FOCOS"]], how = "right", on = "Semana").copy()
	print(f"\n{green}dataset2:\n{reset}{dataset2}")
	print(f"\n{green}dataset2.info:\n{reset}{dataset2.info()}")
	for r in range(_HORIZONTE + 1, _RETROAGIR):
...
def metricas(dataset, previsoes, n, y):
	...
	lista_op = [f"Observado: {dataset['FOCOS'][i]}\nPrevisão (RF): {previsoes[i]}\n" for i in range(n)]
	print("\n".join(lista_op))
	print(f"{red}~{cyan}{reset}"*40)
...
focos_ultimos = focos.copy()#.iloc[-9:,:]
...
if _AUTOMATIZA == True:
	for _CIDADE in cidades:
		try:
			...
			previsao_total[_CIDADE] = previsoes
...
ano_epi = tempo["ano_epi"].iloc[-1 - _HORIZONTE]
print(f"\n{green}DATA EPIDEMIOLÓGICA:\n{reset}{SE}/{ano_epi}\n")
...
ultimas_previsoes = previsao_total.copy()#.iloc[-3:,:]
print(f"\n{green}previsao_total\n{reset}{previsao_total}\n")
...
previsao_total_vdd = previsao_total.drop(columns = "index")
...
if _SALVAR == True:
	...
	previsao_total_csv = f"previsoes_{ano_epi}_focos_vSE{SE}_h{_HORIZONTE}_r{_RETROAGIR}.csv"
	previsao_total_vdd.to_csv(f"{caminho_resultados}{previsao_total_csv}", index = False)
	print(f"\n\n{green}{caminho_resultados}\n{previsao_total_csv}\nSALVO COM SUCESSO!{reset}\n\n")
	print(f"\n\n{green}OS VALORES DAS ÚLTIMAS PREVISÕES SÃO APRESENTADOS ABAIXO:\n{reset}{previsao_total_vdd}\n\n")
