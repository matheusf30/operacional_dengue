...
if _SALVAR == True:
	...
	previsao_pivot_csv = f"previsao_pivot_total_vSE{SE}_h{_HORIZONTE}_r{_RETROAGIR}.csv"
	previsao_total.to_csv(f"{caminho_resultados}{previsao_pivot_csv}", index = False)
	previsao_melt_csv = f"previsao_melt_total_vSE{SE}_h{_HORIZONTE}_r{_RETROAGIR}.csv"
...
for idx, semana_epidemio in enumerate(lista_semanas):
	...
	print(f"\n{green}v_max\n{reset}{v_max}\n")
	print(f"\n{green}levels\n{reset}{levels}\n")
	previsao_melt_poligeo[previsao_melt_poligeo["Semana"] == semana_epidemio["Semana"]].plot(ax = ax, column = "Casos",  legend = True, edgecolor = "white",
		                                                                           label = "Casos", cmap = "YlOrRd", linewidth = 0.05,  legend_kwds = {"extend": "max", "fraction": 0.035, "pad": 0.03, "label": "Número de Casos Prováveis"}, #levels = levels, 
	...
	zero[zero["Semana"] == semana_epidemio["Semana"]].plot(ax = ax, column = "Casos", legend = False, edgecolor = "white", linewidth = 0.05,
...
modelagem inexistente.""",
	...
	nome_arquivo_png = f"CASOS_mapa_preditivo_vSE{SE}_SE{semana_epidemio['SE']}-{semana_epidemio['ano_epi']}.png"
...
if _SALVAR == True:
	os.makedirs(caminho_resultados, exist_ok = True)
	ultimas_previsoes_csv = f"ultimas_previsoes_vSE{SE}_h{_HORIZONTE}_r{_RETROAGIR}.csv"
